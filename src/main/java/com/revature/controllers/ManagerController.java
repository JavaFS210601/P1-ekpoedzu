	...
	public void login(HttpServletRequest req, HttpServletResponse res) throws IOException {
		if(req.getMethod().equals("POST")) { //making sure we actually got a POST request before executing. 
			...
			//control flow to determine what happens in the event of a successful/unsuccessful login--------
						...
						if(ManagerService.login(lDTO.username, lDTO.password)) { //if the username/password sent to the service are valid...
							...
							res.getWriter().print("successful Login  "); //we won't see this message anywhere but postman
